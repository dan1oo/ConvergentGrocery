...
export default function App() {
  ...
  const [sustProd, setSustProd] = useState('No sustainability info yet.')
  const [sustBrand, setSustBrand] = useState('No sustainability info yet.')
  ...
  const sendDataToServer = (data) => {
      fetch('http://192.168.0.137:5000/receive_data', {
          method: 'POST',
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({"variable":data}),
        }).then(response => response.json())
        .then(json => {
          console.log(json)
          setScore(json.score)
          setSustProd(json.sustainability_info_product)
          setSustBrand(json.sustainability_info_brand)
          navigation.navigate("Item", {"score":json.score, "sustainability_product":json.sustainability_info_product, "sustainability_brand":json.sustainability_info_brand })
          //navigation.navigate("TripReport", {"score":json.score})
          return json;
        })
        .catch(error => {
          console.error(error);
